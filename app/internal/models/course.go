...
// Course represents a course entry in the database.
type Course struct {
...
// CreateCourse inserts a new course record and returns its ID.
func CreateCourse(ctx context.Context, tx pgx.Tx, c *Course) (int64, error) {
	var id int64
	err := tx.QueryRow(ctx, `
		INSERT INTO courses (code, title, description, category, credit_hours, is_course_active, rating)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING course_id`,
		c.Code, c.Title, c.Description, c.Category, c.CreditHours, c.IsActive, c.Rating,
	).Scan(&id)
	return id, err
}
...
func GetCourseByID(ctx context.Context, db *pgxpool.Pool, id int64) (*Course, error) {
	row := db.QueryRow(ctx, `
		SELECT course_id, code, title, description, category, credit_hours, is_course_active, rating, created_at
		FROM courses
...
func UpdateCourse(ctx context.Context, tx pgx.Tx, c *Course) error {
	_, err := tx.Exec(ctx, `
		UPDATE courses
		SET code = $1, title = $2, description = $3, category = $4,
		    credit_hours = $5, is_course_active = $6, rating = $7
		WHERE course_id = $8`,
