...
func CreateParticipant(ctx context.Context, tx pgx.Tx, p *Participant) (int64, error) {
    var id int64
    err := tx.QueryRow(ctx, `
        INSERT INTO participant (course_session_id, person_id, status, credit_awarded, note)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING participant_id`,
        p.CourseSessionID, p.PersonID, p.Status, p.CreditAwarded, p.Note,
    ).Scan(&id)
    return id, err
}
...
func GetParticipantByID(ctx context.Context, db *pgxpool.Pool, id int64) (*Participant, error) {
    row := db.QueryRow(ctx, `
        SELECT participant_id, course_session_id, person_id, status, credit_awarded, note, created_at
        FROM participant WHERE participant_id=$1`, id)
...
func UpdateParticipant(ctx context.Context, tx pgx.Tx, p *Participant) error {
    _, err := tx.Exec(ctx, `
        UPDATE participant
        SET course_session_id=$1, person_id=$2, status=$3, credit_awarded=$4, note=$5
